...
def agents_md() -> str:
    return "\n".join(
        [
            ...
            "",
            "Choose one preparation path; the commands below are alternatives, not a checklist.",
            "Bounded work: `kunity-yamae worker-pack --request-file task.json --json`.",
            "It bundles risk, exact source ranges, versions and a Unity recipe in one local call.",
            "Do not rerun context/risk/plans unless scope or observed inputs changed.",
            "",
            "```powershell",
            ...
            "```",
            "",
            "Only when modifying the Python harness repository (not ordinary Unity game edits):",
            "",
            "```powershell",
            "python -m pytest -q",
            "python -m ruff check .",
            ...
            "- Run `kunity-yamae providers doctor --json` at setup or if the environment changed.",
            "- Default report: changed paths, actual checks, blockers; no tutorial or task recap.",
            "- Batch independent reads; use one writer. No agents for deterministic work.",
            (
...
def claude_md() -> str:
    return "\n".join(
        [
            ...
            "Treat the shared inventory as bounded generic semantic signals, not Inspector proof.",
            "",
            "Choose the necessary preparation; do not repeatedly scan unchanged scope.",
            "Bounded work: `kunity-yamae worker-pack --request-file task.json --json`.",
            "",
            "```powershell",
