...
Codex App/CLI와 Claude Code Desktop/CLI용 프로젝트 entrypoint를 설치하려면 다음을 사용합니다.
...
Codex App, Codex CLI, Claude Code Desktop, Claude CLI 진입점과 `local-patch` 오프라인 handoff 준비 상태를 점검합니다.
...
K-Unity-Yamae는 Windows에서 Codex와 Claude의 데스크톱 앱 및 CLI가 같은 Unity 작업 규칙을 읽고, 실제 수정은 guarded local patch 흐름으로 넘기도록 설계되었습니다.
...
이 명령은 `AGENTS.md`, `CLAUDE.md`, `.agents/skills/k-unity-yamae/SKILL.md`, `.claude/skills/k-unity-yamae/SKILL.md`, `.claude/commands/kunity-yamae.md`를 생성합니다.
Codex와 Claude는 각 데스크톱 앱 또는 CLI에서 이 진입 파일을 읽고 Unity 수정 전에 K-Unity-Yamae의 계획, 검증, guarded patch 흐름을 사용합니다.

권장 운영 방식:

- 읽기 전용 분석은 여러 에이전트가 병렬로 수행할 수 있습니다.
- 실제 파일 수정은 `local-patch` handoff와 diff guard를 통과한 patch만 담당합니다.
